  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 text-red-800 p-8 rounded-[2rem] text-center max-w-xl mx-auto my-20">
        <h2 className="text-xl font-black font-outfit uppercase mb-4">
          Error de Conexión
        </h2>
        <p className="text-sm opacity-80 mb-6">{error}</p>
        <button
          onClick={() => window.location.reload()}
          className="bg-red-600 text-white px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-widest"
        >
          Reintentar
        </button>
      </div>
    ...
    <div className="space-y-10 pb-20">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between items-start gap-6">
        <div>
          <div className="flex items-center space-x-3 mb-4">
            ...
            <span className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em]">
              Sistema Operativo {BRAND_CONFIG.shortName}
          ...
          <p className="text-gray-400 font-medium mt-2">
            Bienvenido, Admin. Todo el sistema está operando al 100%.
          </p>
        </div>
        <div className="flex bg-white rounded-2xl p-2 border border-gray-100 shadow-sm sm:w-auto w-full">
          <div className="px-6 py-2 border-r border-gray-50 text-center">
            ...
            <p className="text-[11px] font-black text-green-600 uppercase tracking-tighter">
              Online
            </p>
          </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="bg-gray-100 text-[10px] font-black px-2 py-1 rounded-lg">
              New +2
            </span>
          </div>
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">
          ...
          <Link
            href="/admin/orders"
            className="mt-6 flex items-center text-[10px] font-bold text-gray-900 uppercase tracking-widest group-hover:translate-x-1 transition-transform"
          >
            <span>Gestionar</span>
            <ChevronRight className="w-3 h-3 ml-1" />
          </Link>
        ...
        >
          ...
          <h3 className="text-3xl font-black font-outfit tracking-tighter leading-none">
            $
            {(totalSales / (totalOrders || 1)).toLocaleString("en-US", {
              maximumFractionDigits: 0,
            })}
          </h3>
          <p className="mt-6 text-[10px] font-bold text-blue-200 uppercase tracking-widest">
      ...
      <section>
        <div className="flex items-center justify-between mb-8 px-4">
          ...
          <Link
            href="/admin/orders"
            className="text-xs font-black uppercase text-blue-600 hover:text-black tracking-widest flex items-center transition-colors"
          >
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-50 bg-gray-50/50">
                  <th className="px-10 py-6 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                    ID Orden
                  </th>
                  <th className="px-10 py-6 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                    Cliente
              ...
              <tbody className="divide-y divide-gray-50">
                {recentOrders.length === 0 ? (
                  <tr>
                    <td
                      colSpan={5}
                      className="px-10 py-12 text-center text-gray-400 font-bold uppercase text-[10px] tracking-widest"
                    >
                  ...
                  recentOrders.map((order: any, idx: number) => (
                    ...
                    >
                      <td className="px-10 py-6">
                        <span className="text-sm font-black font-outfit text-blue-600 group-hover:underline cursor-pointer">
                          {order.orderNumber}
                        </span>
                      </td>
                      <td className="px-10 py-6">
                      ...
                      <td className="px-10 py-6">
                        <span className="text-sm font-black font-outfit text-gray-900">
