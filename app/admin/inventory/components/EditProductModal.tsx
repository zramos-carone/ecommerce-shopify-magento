...
import { X, Save, Trash2, Image as ImageIcon, Link as LinkIcon, Package, Tag, DollarSign, AlertCircle } from 'lucide-react';
...
export default function EditProductModal({ product, isOpen, onClose, onUpdate, onDelete }: EditProductModalProps) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          >
            ...
            <div className="px-8 py-6 border-b border-gray-50 flex items-center justify-between bg-white sticky top-0 z-10">
              <div>
                <h2 className="text-xl font-black font-outfit uppercase tracking-tighter">Editar Producto</h2>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">ID: {product.id.slice(-8)}</p>
              </div>
              <button onClick={onClose} className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 hover:bg-red-50 hover:text-red-600 transition-all">
                <X className="w-5 h-5" />
              </button>
            </div>
                ...
                {activeTab === 'details' ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">SKU / Modelo</label>
                      <input 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Precio (MXN)</label>
                      <div className="relative">
                        <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                        <input 
                          type="number" step="0.01" name="price" value={formData.price || 0} onChange={handleChange}
                    ...
                    <div className="space-y-2">
                      ...
                      <input 
                        type="number" name="stock" value={formData.stock || 0} onChange={handleChange}
                  ...
                  <div className="space-y-8">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">URL de la Imagen</label>
                      <div className="relative">
                      ...
                      <p className="text-[9px] text-gray-400 font-bold ml-1">Copia la URL de una imagen externa (Unsplash, Drive, CDN).</p>
                    </div>

                    <div className="p-6 bg-gray-50 rounded-[2rem] border border-dashed border-gray-200">
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 text-center">Previsualización</p>
                    ...
                    <div className="bg-blue-50 p-6 rounded-2xl flex items-start space-x-4 border border-blue-100/50">
                      ...
                      <p className="text-[10px] text-blue-600 font-bold leading-relaxed uppercase tracking-tighter">
                        CONSEJO ESCALABLE: Actualmente usamos URLs externas. En el futuro, este espacio incluirá un botón de "Subir Archivo" para guardar fotos directamente en tu propio storage dedicado.
                      </p>
                    </div>
                  </div>
                )}
