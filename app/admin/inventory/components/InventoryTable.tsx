    ...
    try {
      ...
      if (result.success && result.results[0]?.success) {
        setProducts((prev) =>
          prev.map((p) =>
            p.id === productId
              ? { ...p, stock: result.results[0].newStock }
              : p,
          ),
        );
      }
          ...
          <tbody className="divide-y divide-gray-50">
            {filteredProducts.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className="px-10 py-20 text-center text-gray-300"
                >
                ...
                >
                  ...
                  <td className="px-10 py-6">
                    <div className="flex items-center space-x-3">
                      <button
                        onClick={(e) =>
                          handleQuickStockAdjustment(e, product.id, -1)
                        }
                        disabled={
                          product.stock <= 0 || updatingId === product.id
                        }
