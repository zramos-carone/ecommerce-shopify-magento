  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/mayoristas/raw?q=${encodeURIComponent(searchTerm)}&limit=20`,
      );
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between items-start gap-6">
        <div>
          ...
          <p className="text-gray-400 font-medium mt-2 max-w-2xl">
            Busca productos en stock real. Impórtalos y úete al{" "}
            <a
            ...
            para agregar tu foto personalizada y publicarlos.
          </p>
        </div>
        {/* Banner de flujo */}
        <div className="flex items-center space-x-4 bg-amber-50 border border-amber-200 rounded-2xl px-6 py-4 text-amber-800">
          ...
          <div>
            ...
            <p className="text-xs font-medium mt-1">
              1. Importar &rarr; 2.{" "}
              <a href="/admin/inventory" className="underline font-bold">
                Añadir tu foto en el Inventario
              </a>{" "}
              &rarr; 3. El producto aparece en la Tienda
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="sticky top-2 z-20">
        {error && (
          <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-[1.5rem] flex items-center space-x-3 text-red-600">
            ...
            >
              ...
              <div className="flex-1 flex flex-col">
                <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2">
                  {product.brand || "Varios"}
                </span>
                <h3 className="text-sm font-bold text-gray-900 line-clamp-2 leading-tight h-10">
                  {product.name}
                </h3>
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex flex-col">
                    ...
                    <span className="text-lg font-black font-outfit text-gray-900">
        ...
        {results.length === 0 && !isSearching && (
          <div className="col-span-full py-20 text-center border-2 border-dashed border-gray-100 rounded-[3rem]">
            ...
            <p className="text-lg font-black text-gray-300 uppercase tracking-widest">
              Inicia una búsqueda global
            </p>
            <p className="text-sm text-gray-400 font-medium">
              Conecta con todos los mayoristas de MaxTech
            </p>
          </div>
        )}
      </div>
