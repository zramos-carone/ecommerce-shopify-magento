 ...
 * - ❌ Nunca menciona al mayorista al cliente → toda la identidad es MaxTech
...
export async function GET(request: NextRequest) {
  try {
    ...
    const localOverrides = skus.length > 0
      ? await prisma.product.findMany({
          where: { sku: { in: skus } },
          select: { sku: true, name: true, image: true, price: true, brand: true }
        })
      : []
    ...
    const products: MayoristaProduct[] = mayoristaResults.products.map(product => {
      ...
      return {
        ...
        // Identidad: siempre MaxTech
        mayorista: 'MAXTECH',
      }
    })

    console.log(`💎 [BOUTIQUE_HYBRID] Query: "${q}". Total: ${products.length}. Con imagen: ${products.filter(p => p.imageUrl).length}`)
    ...
    return NextResponse.json(result, { status: 200, headers: { 'X-RateLimit-Remaining': String(rateLimiter.getRemaining(ip)) } })
  } catch (error) {
