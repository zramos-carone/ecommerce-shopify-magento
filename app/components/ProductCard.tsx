...
export function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  const getStockLabel = () => {
    ...
    return {
      text: "En Stock - Entrega Inmediata",
    ...
    <div className="group relative overflow-hidden rounded-[2.5rem] border border-gray-100 bg-white hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
      <Link href={`/catalog/${product.id}`} className="block">
        ...
        <div className="relative aspect-square bg-gray-50 m-2 rounded-[2rem] overflow-hidden flex items-center justify-center">
          ...
          <div
            className={`absolute top-4 left-4 flex items-center space-x-2 px-3 py-1.5 rounded-full border backdrop-blur-md shadow-sm ${stockInfo.color}`}
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${stockInfo.dot} ${product.stock > 0 ? "animate-pulse" : ""}`}
            ></span>
        ...
        <div className="p-6 pt-2">
          ...
          <div className="flex flex-col mb-6">
            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter leading-none mb-2 underline decoration-blue-200 decoration-2 underline-offset-4">
              Precio Total
            </span>
            <span className="text-2xl font-black text-gray-900 font-outfit tracking-tighter leading-none">
              ...
              <span className="text-xs text-gray-400 ml-1 font-medium font-sans underline-none">
                MXN
              </span>
            </span>
          </div>
